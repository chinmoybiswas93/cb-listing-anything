...
$breadcrumb_post_id = isset( $post_id ) ? absint( $post_id ) : ( is_singular( \CBListingAnything\Config\PostType::POST_TYPE ) ? get_the_ID() : 0 );
...
$is_listing_context = $is_archive || $is_category || $is_tag || ( $breadcrumb_post_id && \CBListingAnything\Config\PostType::POST_TYPE === get_post_type( $breadcrumb_post_id ) );
?>
<nav class="cb-listing-breadcrumb"<?php echo $breadcrumb_style; ?> aria-label="<?php esc_attr_e( 'Breadcrumb', 'cb-listing-anything' ); ?>">
	...
	<?php if ( $is_listing_context ) : ?>
		...
		<a href="<?php echo esc_url( get_post_type_archive_link( \CBListingAnything\Config\PostType::POST_TYPE ) ); ?>"<?php echo $link_style; ?>><?php esc_html_e( 'Listings', 'cb-listing-anything' ); ?></a>
		...
		<?php elseif ( $is_tag && $current_term ) : ?>
			<span class="cb-listing-breadcrumb__sep">/</span>
			<span class="cb-listing-breadcrumb__current"<?php echo $current_style; ?>><?php echo esc_html( $current_term->name ); ?></span>
		<?php elseif ( ! $is_archive && $breadcrumb_post_id && \CBListingAnything\Config\PostType::POST_TYPE === get_post_type( $breadcrumb_post_id ) ) : ?>
			<?php
			// Single listing page: show category and post title
			$categories = get_the_terms( $breadcrumb_post_id, \CBListingAnything\Config\Taxonomies::CATEGORY_TAXONOMY );
